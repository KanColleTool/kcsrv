import util
from db import db,Equipment, Ship,Stats,Resources,Remodel,Item
...
def items():
    ...
    count = 0
    for item in items:
        old = db.session.query(Item).filter(Item.api_id==item["api_id"]).first()
        ...
        db.session.add(i)
        print("Merged equip {} - {}".format(i.api_id,i.name))
        count += 1
    print("Updated Item database, {} entries added.".format(count))
    db.session.commit()
